...
export class Row extends Component {
  ...
  render() {
    ...
    return data ? (
      <View style={[height && { height }, width && { width }, styles.row, style, {flexDirection: 'row'}]}>
        <View style={[styles.row, style, {flex: 1, backgroundColor: "#ff6666"}]}>
        ...
        <View style={[styles.row, style, {flex: 1, backgroundColor: "#80bfff"}]}>
        {data.slice(0, 3).map((item, i) => {
          const flex = flexArr && flexArr[i];
          const wth = widthArr && widthArr[i];
          return <Cell key={i} data={item} width={wth} height={height} flex={flex} textStyle={textStyle} {...props} />;
